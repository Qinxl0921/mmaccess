...
func (caecol *CAECollection) PushFile(trailId, fileName, filePath string) error {
	...
	*/
	// endTime_int64 := time.Now().Unix()
	// endTime := strconv.FormatInt(endTime_int64, 10)
	// sendendinfo := "trailId=" + trailId + "&fileName=" + fileName + "&startTime=" + startTime + "&endTime=" + endTime + "&status=finish"
	// if _, err := http.Get("http://localhost:8080/websocket?" + sendendinfo); err != nil {
	// 	fmt.Println("已发送finish=====")
	// 	fmt.Println("没有连接dbm")
	// }
	...
	/*
		...
		if _, err := http.Get("http://127.0.0.1:8080/websocket?" + sendendinfo); err != nil {
			fmt.Println("信息已发送=====")
			fmt.Println("没有连接dbm")
		}
		fmt.Print("=====上传完成=====")
...
func main() {
	...
	cae.PushFile("001", "实验文件3", "C:/Users/ASUS/Desktop/yolov5口罩检测.zip")
